...
lineaccesstoken = 'SCqJNXe1ojrEZappdn78aFtTuoLkoK4S8/yNl5irlsViNOcTapJNPdOj21jBKi9nep1YdIw9T6owGdg3dGJZMhSrJmGOftI3u/NTdVy+q3+ubWRbVuPek2Gbp6HgibcKbr77to5bYYmpePewG5KEFwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="มาอีกแล้วหรอ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวยัง") :
            replyObj = TextSendMessage(text="เป็นบอทค่าาากินข้าวไม่ได้ไม่ฉลาดเลยนะ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินน้ำมันมั้ย") :
            replyObj = TextSendMessage(text="ไม่ใช่หุ่นยนต์ค่ะ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
